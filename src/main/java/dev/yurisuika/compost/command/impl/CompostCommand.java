package dev.yurisuika.compost.command.impl;
...
import dev.yurisuika.compost.mixin.command.arguments.ItemInputAccessor;
        ...
        dispatcher.register(literal("compost")
                .then(literal("config")
                        ...
                        .then(literal("reload")
                                .executes(context -> {
                                    loadConfig();
                                    context.getSource().sendSuccess(new TranslationTextComponent("commands.compost.config.reload"), true);
                                    return 1;
                                })
                        )
                        .then(literal("reset")
                                .executes(context -> {
                                    ...
                                    setShuffle(true);
                                    context.getSource().sendSuccess(new TranslationTextComponent("commands.compost.config.reset"), true);
                                    return 1;
                                })
                        )
                )
                .then(literal("shuffle")
                        ...
                        .then(literal("query")
                                .executes(context -> {
                                    context.getSource().sendSuccess(new TranslationTextComponent("commands.compost.shuffle.query", config.shuffle), false);
                                    return 1;
                                })
                        )
                        .then(literal("set")
                                .then(argument("value", BoolArgumentType.bool())
                                        .executes(context -> {
                                            ...
                                            setShuffle(value);
                                            context.getSource().sendSuccess(new TranslationTextComponent("commands.compost.shuffle.set", value), true);
                                            return 1;
                                        })
                                )
                        )
                ...
                .then(literal("groups")
                        ...
                        .then(literal("query")
                                .executes(context -> {
                                    context.getSource().sendSuccess(new TranslationTextComponent("commands.compost.groups.query", config.items.length), false);
                                    return 1;
                                })
                        )
                        .then(literal("get")
                                .executes(context -> {
                                    for (Group group : config.items) {
                                        ...
                                        context.getSource().sendSuccess(new TranslationTextComponent("commands.compost.groups.get", ArrayUtils.indexOf(config.items, group) + 1, itemStack.getDisplayName(), new DecimalFormat("0.###############").format(BigDecimal.valueOf(chance).multiply(BigDecimal.valueOf(100))), min, max), false);
                                    }
                                    return 1;
                                })
                        )
                        .then(literal("reverse")
                                .executes(context -> {
                                    reverseGroups();
                                    context.getSource().sendSuccess(new TranslationTextComponent("commands.compost.groups.reverse"), true);
                                    return 1;
                                })
                        )
                        .then(literal("shuffle")
                                ...
                                .then(argument("group", IntegerArgumentType.integer(1))
                                        .executes(context -> {
                                            ...
                                            if (range > config.items.length) {
                                                ...
                                                int min = Math.min(Math.min(group.min, maxCount), max);
                                                removeGroup(number);
                                                context.getSource().sendSuccess(new TranslationTextComponent("commands.compost.group.remove", itemStack.getDisplayName(), new DecimalFormat("0.###############").format(BigDecimal.valueOf(chance).multiply(BigDecimal.valueOf(100))), min, max), true);
                                                return 1;
                                            }
                                        })
                                )
                                                                ...
                                                                .then(argument("max", IntegerArgumentType.integer(1, 64))
                                                                        .executes(context -> {
                                                                            ...
                                                                            if (range > config.items.length) {
                                                                                ...
                                                                                context.getSource().sendSuccess(new TranslationTextComponent("commands.compost.group.set", itemStack.getDisplayName(), new DecimalFormat("0.###############").format(BigDecimal.valueOf(chance).multiply(BigDecimal.valueOf(100))), min, max), true);
                                                                                return 1;
                                                                            }
                                                                        })
                                                                )
