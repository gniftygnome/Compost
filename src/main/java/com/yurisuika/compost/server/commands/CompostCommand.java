    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(literal("compost")
                .then(literal("config")
                        ...
                        .then(literal("reset")
                                .executes(context -> {
                                    ...
                                    setShuffle(true);
                                    context.getSource().sendSuccess(new TranslatableComponent("commands.compost.config.reset"), true);
                                    return 1;
                                })
                        )
                )
                .then(literal("shuffle")
                        ...
                        .then(literal("query")
                                .executes(context -> {
                                    context.getSource().sendSuccess(new TranslatableComponent("commands.compost.shuffle.query", config.shuffle), false);
                                    return 1;
                                })
                        )
                        .then(literal("set")
                ...
                .then(literal("groups")
                        ...
                        .then(literal("query")
                                .executes(context -> {
                                    context.getSource().sendSuccess(new TranslatableComponent("commands.compost.groups.query", config.items.length), false);
                                    return 1;
                                })
                        )
                        .then(literal("get")
                                ...
                                .then(argument("group", IntegerArgumentType.integer(1))
                                        .executes(context -> {
                                            ...
                                            if (range > config.items.length) {
                                                context.getSource().sendFailure(new TranslatableComponent("commands.compost.group.failed", range, config.items.length));
                                                return 0;
                                            } else {
                                                ...
                                                .then(argument("min", IntegerArgumentType.integer(0, 64))
                                                        .then(argument("max", IntegerArgumentType.integer(1, 64))
                                                                .executes(context -> {
                                                                    ...
                                                                    StringBuilder stringBuilder = new StringBuilder(Objects.requireNonNull(arg.getItem().builtInRegistryHolder().unwrapKey().map(ResourceKey::location).orElseGet(() -> ResourceLocation.tryParse("unknown[" + arg.getItem() + "]"))).toString());
